...
$file_path = '/Users/aaronrowe/Desktop/marketing_agencies.json';

$db_user = 'root';
$db_pass = 'root';
$db_name = 'wrc_marketing_rebuild';
$db_host = 'localhost';
...
$acf_agency_field_names_map = $dbi->get_results("select meta_key, meta_value from wp_posts as p left join wp_postmeta as pm on p.id = pm.post_id where p.post_type = 'agent' and pm.meta_key like '\_%' and pm.meta_value like 'field\_%' group by meta_key;");
...
//$mutuals_map = buildObjectIDMap($input_data['mutuals'], intval($next_post_id[0]->Auto_increment));
...
$agency_acf_map = array(
  'agent_address_line_1' => 'address',
  'agent_phone' => 'phone',
  'agent_url' => 'website'
);
...
//var_dump(rtrim($agency_queries['insert_query'], ", ")); die;
...
function buildObjectInsertStatements($object_data, $table_name, $meta_table_name, $meta_table_foreign_key, $meta_table_reference_data, $meta_table_column_name_maps, $meta_acf_map, $post_type_map){
  $old_site_path = '//wrc-marketing.loc/';
  $new_site_path = '//wrc-marketing-rebuild.loc/';
      ...
      if($key !== 'ID'){
        ...
        if(strpos($key, 'meta_') === false && !array_key_exists($key, $meta_table_reference_data)){
          ...
          if(!empty($value) && $value !== "null"){
            ...
            //address line 2 does not exist in new db, break up into city, state, zip
            if($meta_key === 'address_line_2'){
